...
class Application(tk.Frame):
    ...
    class FrameEntry(tk.Frame):
        def __init__(self, arrayIngredient: r.RecipeComponent, master=None):
            super().__init__(master)
            #self.pack()
            self.master = master
            ...
            self.entry_name.grid(row = 0, column=0, padx=0, pady=5)
            self.button_remove.grid(row = 0, column=1, padx=0, pady=5)
            if arrayIngredient.isRecipe():
                self.createSubRecipeFrames(arrayIngredient.ingredient)
                self.currentIngredients = arrayIngredient.ingredient.componentList
